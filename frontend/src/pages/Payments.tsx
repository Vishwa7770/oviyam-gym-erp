            ...
            <tbody>
              {payments.map(p => (
                <tr key={p.id} className="border-b border-black/5 dark:border-white/5 last:border-0 hover:bg-black/2 dark:hover:bg-white/2 text-xs">
                  ...
                  <td className="p-4 text-right font-semibold">₹{p.final_amount.toFixed(2)}</td>
                  <td className="p-4 text-right font-black text-emerald-500">₹{p.paid_amount.toFixed(2)}</td>
                  <td className="p-4 text-right font-bold text-red-500">₹{p.pending_amount.toFixed(2)}</td>
                  <td className="p-4">
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">{settings.currency || '₹'}{activeInvoice.amount.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600">Discount Applied:</span>
                    <span className="font-bold text-red-500">-{settings.currency || '₹'}{activeInvoice.discount.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between border-t pt-2 font-bold text-sm">
                    <span>Final Payable Amount:</span>
                    <span>{settings.currency || '₹'}{activeInvoice.final_amount.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between font-black text-emerald-600 text-sm">
                    <span>Total Amount Paid:</span>
                    <span>{settings.currency || '₹'}{activeInvoice.paid_amount.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between border-t pt-2 font-bold text-red-500">
                    <span>Dues Balance Pending:</span>
                    <span>{settings.currency || '₹'}{activeInvoice.pending_amount.toFixed(2)}</span>
                  </div>
                </div>
              </div>
